...
Integralną część pracy stanowią opracowane kody -- zarówno w języku {\em C++}, jak i w postaci tak zwanych {\em blueprintów}.}
...
Podczas pracy nad projektem korzystano z systemu kontroli wersji {\em Git} \cite{git}. 
Został on opracowany przez Linusa Torvaldsa, aby ułatwić zarządzanie rozwojem jądra systemu Linux.
Strona manuala polecenia \verb+git+ nazywa ów system żartobliwie ,,głupim protokolantem treści''. Jest to aluzja do ,,filozofii'' tego systemu, odróżniającej go od innych narzędzi kontroli wersji, a zwłaszcza do prostoty użytych struktur danych.
Nacisk położony został na rozproszony charakter, wydajność, skalowalność oraz możliwość pracy nad projektem przez wiele osób równocześnie.
Oprogramowanie to wspiera zatem pracę zespołową programistów -- między innymi poprzez ułatwione rozwiązywanie tak zwanych konfliktów, czyli sytuacji, w których kilku programistów jednocześnie modyfikuje ten sam plik tekstowy. Ponadto {\em Git} umożliwia przechowywanie plików projektowych na wspólnym serwerze, do którego zespół ma dostęp, a także śledzenie historii zmian czy ,,rozgałęzianie`` (ang. {\em branching}) projektu, na przykład w przypadku implementowania nowych funkcji, które mogłyby uszkodzić dotychczasową, działającą wersję projektu lub gdy trzeba poprawić krytyczny błąd w starej wersji programu, nie włączając jednak funkcjonalności z nowej wersji..
\newline \indent Katalog zarządzany przez system {\em Git} nazywa się ,,repozytorium``, a jego wersje przechowywane są na wspólnym serwerze oraz lokalnie na komputerach programistów. Praca z {\em Gitem} polega głównie na pobieraniu (polecenie {\em pull}) zmian z serwera do lokalnego repozytorium, zatwierdzaniu (polecenie {\em commit}) swoich zmian oraz wysyłaniu (polecenie {\em push}) ich do repozytorium na serwerze.
Repozytorium niniejszego projektu dostępne jest pod adresem \url{https://github.com/mszadko/thesis\_1}.
\newline \indent W rozdziale \ref{problemy} opisano problemy, na jakie natrafiliśmy, wykorzystując {\em Git} do pracy nad projektami UE4.
...
Gdy odległość postaci od ogniska będzie mniejsza niż promień sfery, postać zostanie dodana do listy leczonych postaci i liczba jej punktów zdrowia zacznie wzrastać.
Jeśli natomiast postać zanadto zbliży się do ogniska, dozna ,,oparzeń'', czyli zacznie tracić punkty zdrowia, zamiast je odzyskiwać.
Oczywiście, zaimplementowane ognisko jest tylko przykładem. W analogiczny (i równie prosty) sposób można, za pomocą blueprintów,  dodać do gry inne elementy środowiska, wpływające w określony sposób na postacie, które w danym miejscu (i czasie) się znalazły.
...
Społeczność ta dyskutuje głównie na oficjalnym forum\cite{ueforum} i w sekcji pytań\cite{ueanswerhub}.
...
\newline \indent W katalogu DOKUMENTY umieszczono pracę dyplomową w formatach PDF i TEX oraz plik z tytułem, streszczeniem i słowami kluczowymi w formacie ODT. 
...
\bibitem{git} \textit{Git. Strona domowa projektu}. \url{https://git-scm.com} (dostęp 15.12.2017)

\bibitem{git-man} \textit{Strona manuala dla polecenia git}. \url{https://linux.die.net/man/1/git} (dostęp 15.12.2017)
...
\bibitem{ueanswerhub}
\textit{Unreal Engine AnswerHub }. \url{https://answers.unrealengine.com/index.html}  (dostęp 17.01.2018)

\bibitem{ueforum}
\textit{Unreal Engine Forum}. \url{https://forums.unrealengine.com/}  (dostęp 17.01.2018)
